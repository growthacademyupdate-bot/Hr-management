...
import { type PayrollInput, type PayrollResult } from "@/lib/payroll";
...
type AttendanceSummary = { totalDaysInMonth: number; activeDays: number; presentDays: number; paidLeaveDays: number; unpaidLeaveDays: number; absentDays: number; unpaidDays: number; payableDays: number; monthlyInHandSalary: number; perDaySalary: number; unpaidDeduction: number; finalInHandSalary: number };
type Slip = PayrollResult & AttendanceSummary & { id: string; employeeId: string; salaryMonth: number; salaryYear: number; employeeSnapshot: { name: string; id: string; department: string; designation: string; joiningDate: string; email: string; } };
...
export default function SalarySlipsPage() {
  ...
  const [attendancePreview, setAttendancePreview] = useState<AttendanceSummary | null>(null);
  ...
  useEffect(() => {
    if (!form.employeeId || !form.salaryMonth || !form.salaryYear) { setAttendancePreview(null); return; }
    let cancelled = false;
    fetch(`/api/salary-slips/attendance/${encodeURIComponent(form.employeeId)}?month=${form.salaryMonth}&year=${form.salaryYear}`, { headers })
      .then(async (response) => { const data = await response.json(); if (!response.ok) throw new Error(data.error || "Could not calculate attendance payroll"); return data.data as AttendanceSummary; })
      .then((data) => { if (!cancelled) setAttendancePreview(data); })
      .catch(() => { if (!cancelled) setAttendancePreview(null); });
    return () => { cancelled = true; };
  }, [form.employeeId, form.salaryMonth, form.salaryYear, headers]);
  ...
  async function generate() {
    if (!form.employeeId || !attendancePreview) { toast.error("Select an employee and wait for attendance calculation"); return; }
    setLoading(true);
    try {
  ...
  return <div className="space-y-6">
    ...
    {canManage && <div className="grid gap-6 xl:grid-cols-[minmax(0,1.1fr)_minmax(360px,.9fr)]">
      <Card><CardHeader><CardTitle className="flex items-center gap-2"><WalletCards className="h-5 w-5 text-primary" /> Generate salary slip</CardTitle></CardHeader><CardContent className="space-y-5">
        ...
        <div className="grid gap-3 sm:grid-cols-2"><Field label="Annual CTC (from employee)" value={employee ? (employee.salary * 12).toFixed(2) : ""} onChange={() => undefined} disabled /><Field label="Monthly in-hand salary" value={attendancePreview?.monthlyInHandSalary?.toFixed(2) || ""} onChange={() => undefined} disabled /></div>
        <div className="grid gap-3 sm:grid-cols-4"><Field label="HRA" value={form.hra || 0} onChange={(value) => setNumber("hra", value)} /><Field label="Conveyance" value={form.conveyance || 0} onChange={(value) => setNumber("conveyance", value)} /><Field label="Medical" value={form.medicalAllowance || 0} onChange={(value) => setNumber("medicalAllowance", value)} /><Field label="Special allowance" value={form.specialAllowance || 0} onChange={(value) => setNumber("specialAllowance", value)} /></div>
        <div className="grid gap-3 sm:grid-cols-4"><Field label="Total days" value={attendancePreview?.totalDaysInMonth || ""} onChange={() => undefined} disabled /><Field label="Payable days" value={attendancePreview?.payableDays || ""} onChange={() => undefined} disabled /><Toggle label="PF enabled" checked={Boolean(form.pfEnabled)} onCheckedChange={(value) => update("pfEnabled", value)} /><Toggle label="ESI enabled" checked={Boolean(form.esiEnabled)} onCheckedChange={(value) => update("esiEnabled", value)} /></div>
      ...
      <div className="space-y-4"><Preview result={attendancePreview ? { monthlyCTC: attendancePreview.monthlyInHandSalary, grossSalary: attendancePreview.monthlyInHandSalary, totalDeductions: attendancePreview.unpaidDeduction, netSalary: attendancePreview.finalInHandSalary, amountInWords: "", } as PayrollResult : null} employee={employee} month={form.salaryMonth} year={form.salaryYear} onPrint={() => window.print()} /><AttendanceBreakdown result={attendancePreview} /></div>
    </div>}
    {selected && <div className="space-y-4"><Preview result={selected} employee={db.employees.find((item) => item.id === selected.employeeId)} month={selected.salaryMonth} year={selected.salaryYear} onPrint={() => downloadSlip(selected.id)} /><AttendanceBreakdown result={selected} /></div>}
...
function AttendanceBreakdown({ result }: { result: AttendanceSummary | null }) { if (!result) return null; return <Card><CardHeader><CardTitle>Attendance-based salary calculation</CardTitle></CardHeader><CardContent className="grid grid-cols-2 gap-3 text-sm sm:grid-cols-4"><Line label="Total days in month" value={String(result.totalDaysInMonth)} /><Line label="Present days" value={String(result.presentDays)} /><Line label="Paid leave days" value={String(result.paidLeaveDays)} /><Line label="Unpaid leave days" value={String(result.unpaidLeaveDays)} /><Line label="Absent days" value={String(result.absentDays)} /><Line label="Total unpaid days" value={String(result.unpaidDays)} /><Line label="Payable / working days" value={String(result.payableDays)} /><Line label="Per day salary" value={money(result.perDaySalary)} /><Line label="Unpaid deduction" value={money(result.unpaidDeduction)} /><Line label="Final in-hand salary" value={money(result.finalInHandSalary)} strong /></CardContent></Card>; }
