...
import {
  ...
  Bell, LogOut, Menu, Search, Sun, CalendarDays, Receipt, WalletCards, X, ClipboardList, FileText
} from "lucide-react";
...
const ICONS: Record<string, React.ComponentType<{ className?: string }>> = {
  LayoutDashboard, Users, CalendarCheck, ListTodo, CalendarOff, BarChart3, Settings, Activity, User: UserIcon, CalendarDays, Bell, Receipt, ClipboardList, FileText
};
        ...
        <header className="sticky top-0 z-20 h-16 bg-card/95 backdrop-blur border-b flex items-center gap-4 px-4 lg:px-6">
          ...
          <div className="flex items-center gap-2 ml-auto">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-80 sm:w-96 p-0 overflow-hidden shadow-xl border border-border">
                <div className="flex items-center justify-between px-3.5 py-2.5 border-b border-border bg-popover">
                  <div className="flex items-center gap-2">
                    <span className="font-semibold text-sm">Notifications</span>
                    {unreadCount > 0 && (
                      <Badge className="text-[10px] px-1.5 py-0.5 h-4 bg-primary text-primary-foreground">
                        {unreadCount} new
                      </Badge>
                    )}
                  </div>
                ...
                <div className="max-h-[380px] sm:max-h-[420px] overflow-y-auto p-1 divide-y divide-border/40 overscroll-contain">
                  {notifications.length === 0 ? (
                    ...
                    notifications.map((n) => (
                      <DropdownMenuItem 
                        ...
                        className={`py-3 px-3 cursor-pointer hover:bg-muted/80 transition-colors rounded-md my-0.5 ${
                          !n.isRead ? "bg-muted/40 font-medium" : ""
                        }`}
                      >
                        <div className="space-y-1 relative w-full pr-4">
                          ...
                          <div className="text-sm font-semibold leading-tight pr-3">{n.title}</div>
                ...
                {notifications.length > 0 && (
                  <div className="p-2 border-t border-border bg-muted/20 text-center">
                    <Link 
                      href="/notifications" 
                      className="text-xs text-primary hover:underline font-medium block py-0.5"
                    >
